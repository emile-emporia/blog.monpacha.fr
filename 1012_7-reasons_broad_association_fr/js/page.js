...
})();/* ===== keep-45-don-video.js — bloc don : tap pour le son (autoplay muet par défaut) ===== */
(function(){
  var box=document.getElementById('pdvid-don'); if(!box) return;
  var v=box.querySelector('video'); if(!v) return;
  function toggle(){
    if(box.classList.contains('sound-on')){
      v.muted=true; box.classList.remove('sound-on');
    } else {
      v.muted=false; try{v.currentTime=0;}catch(e){}
      var p=v.play(); if(p&&p.catch){p.catch(function(){v.muted=true;box.classList.remove('sound-on');});}
      box.classList.add('sound-on');
    }
  }
